...
export default function Home() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newRequest = {
      id: 'r' + Date.now(),
      userId: currentUser?.id || 'guest', // For non-logged users use 'guest'
      status: 'active',
      from: {
    ...
    if (!currentUser) {
      // For non-logged users, replace all guest requests with the new one (only one request allowed)
      const loggedUserRequests = existingRequests.filter((req: any) => req.userId !== 'guest');
      loggedUserRequests.push(newRequest);
      localStorage.setItem('transportRequests', JSON.stringify(loggedUserRequests));
    } else {
      // For logged users, add to the list normally
